...
#chapter-lead[
  実行系の性能と対応機能は、更新によって変わる。
  実装するときは、採用するモデルやアセットのバージョンを記録し、配布元の資料と仕様書を改めて確認してほしい。
...
- #source-link([ExecuTorch 公式リポジトリ], "https://github.com/pytorch/executorch"):第 1 章の実行系の選定で比較に挙げた、PyTorch の端末向け実行系。
- #source-link([MLC LLM 公式リポジトリ], "https://github.com/mlc-ai/mlc-llm"):第 1 章の実行系の選定で比較に挙げた、モデルを端末ごとに事前コンパイルする実行系。
- #source-link([LiteRT-LM], "https://developers.google.com/edge/litert-lm"):第 1 章の実行系の選定で比較に挙げた、Google の端末向け LLM 実行層。
...
- #source-link([SenseVoice], "https://github.com/FunAudioLLM/SenseVoice"):多言語音声認識モデル。発話単位の感情・音響イベントのラベルを返す。非自己回帰の構造と規模の出典。
...
- #source-link([misakid], "https://github.com/ProjectKokage/misakid"):Kokoro 系のモデルが使う読みの解析エンジン Misaki の Dart 移植。日本語の Open JTalk 系の解析を含む、こかげの解析器の実体。
...
- #source-link([Kokoro 82M タイムスタンプ対応 ONNX], "https://huggingface.co/onnx-community/Kokoro-82M-v1.0-ONNX-timestamped/tree/dd4401a9add81ac692d20e240d22ec9dda82cc29"):こかげがバージョンを固定した、波形とトークン時刻を返す音声合成モデル。本文の量子化版(model_quantized.onnx)を含む配布。
- #source-link([fonix], "https://github.com/ProjectKokage/fonix"):ONNX Runtime を Dart と Flutter から使う、こかげの自前ラッパー。音声合成の実行に使う。
...
- #source-link([Flutter Scene], "https://pub.dev/packages/flutter_scene"):採用した Flutter 向けの 3D描画エンジン。こかげはバージョンを固定し、表情の頂点合成を自前で補っている。
- #source-link([flvtterm], "https://github.com/ProjectKokage/flvtterm"):VRM と VRMA を Dart だけで読み込んで動かす、描画エンジンに依存しないこかげの自前ライブラリ。Flutter Scene への接続も含む。
...
- #source-link([AITuber OnAir], "https://github.com/shinshin86/aituber-onair"):設計の部分的な参照元。演出の着想は借り、時計と所有権の設計は借りない。
...
- #source-link([HY-Motion-1.0 公式リポジトリ], "https://github.com/Tencent-Hunyuan/HY-Motion-1.0"):テキストから人型モーションを生成するモデル。クリップの生成に開発機で使用し、端末には載せない。
